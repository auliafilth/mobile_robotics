...
void DesStatePublisher::pub_next_state(bool lidar_alarm) {
    // first test if an e-stop has been triggered
    if (e_stop_trigger_) {
        e_stop_trigger_ = false; //reset trigger
        //compute a halt trajectory
        //double current_velocity = current_des_state_.twist.twist.linear.x; // THIS IS ASSUMING MOBOT IS GOING FORWARDS WHEN AN ESTOP IS NEEDED
        //des_state_vec_.clear(); // DELETE this later? Idk if .clear() is a function.
        trajBuilder_.build_braking_traj(current_pose_, current_des_state_, des_stopping_state_vec_);       // REPLACED des_sate_vec_ with des_stopping_state_vec_
        motion_mode_ = HALTING;
        stopping_traj_pt_i_ = 0; //replaced traj_pt_i with stopping_traj_pt_i_
        npts_traj_ = des_stopping_state_vec_.size();
    }
    ...
    switch (motion_mode_) {
        ...
        case HALTING: //e-stop service callback sets this mode
            ...
            //ROS_INFO("HALTING!");
            current_des_state_ = des_stopping_state_vec_[stopping_traj_pt_i_];
            ...
            if (stopping_traj_pt_i_ >= npts_traj_) { //here if completed all pts of braking traj
        ...
        case PURSUING_SUBGOAL: //if have remaining pts in computed traj, send them
            // first check if there's something in front of us
            if (lidar_alarm){
                e_stop_trigger_ = true; // this should send us to breaking
                lidar_alarm = false; // reset lidar alarm
            }
