...
// Do we need to make a subscriber?
/*
...
void TrajBuilder::build_braking_traj(geometry_msgs::PoseStamped start_pose, // THIS FUNCTION ASSUMES THERE IS NO TURNING!!
        nav_msgs::Odometry current_des_state,                                // ADDED CURRENT VELOCITY
        std::vector<nav_msgs::Odometry> &vec_of_states) {
    //FINISH ME!
    ROS_INFO("WE ARE IN BUILD_BRAKING_TRAJ!");
    double x_start = start_pose.pose.position.x;
    double y_start = start_pose.pose.position.y;
    ...
    des_state.header = start_pose.header; //really, want to copy the frame_id
    des_state.pose.pose = start_pose.pose; //start from here
    des_state.twist.twist = current_des_state.twist.twist; // THIS IS BASED OFF OF THE CURRENT TWIST
    
    double t_ramp = current_des_state.twist.twist.linear.x / accel_max_; // TIME IT TAKES TO COME TO A HALT = DV/A
    int npts_ramp = round(t_ramp / dt_);

    double x_des = x_start; //start from here
    double y_des = y_start;
    double speed_des = current_des_state.twist.twist.linear.x; // CURRENT SPEED IS FIRST SPEED

    double avg_vel = current_des_state.twist.twist.linear.x / 2; // AVERAGE VELOCITY
    double x_end = avg_vel * t_ramp;
    double y_end = y_start; // THIS... SEEMS WRONG
    double dx = x_end - x_start;
    double dy = y_end - y_start;
    double psi_des = atan2(dy, dx);
...
// OLD STUFF I COPIED FROM
    /*
    // same as end of triangle...
    double x_start = start_pose.pose.position.x;
    double y_start = start_pose.pose.position.y;
    ...
    des_state.header = start_pose.header; //really, want to copy the frame_id
    des_state.pose.pose = start_pose.pose; //start from here
    des_state.twist.twist = halt_twist_; // THIS IS NOT TRUE. NEED TO FIND START_TWIST
    double trip_len = sqrt(dx * dx + dy * dy);
    double t_ramp = sqrt(trip_len / accel_max_);
    int npts_ramp = round(t_ramp / dt_);
    double v_peak = accel_max_*t_ramp; // could consider special cases for reverse motion
    ...
    */
    ...
    // Hard Stop Below (BAD)
    //des_state.twist.twist.linear.x = 0; // lol STOP


    //make sure the last state is precisely where requested, and at rest:
    // des_state.pose.pose = end_pose.pose; //REMOVED BECAUSE WE DON'T HAVE AN OFFICIAL END POSE, THOUGH WE DO HAVE X AND Y
    des_state.twist.twist = halt_twist_; // insist on starting from rest
    vec_of_states.push_back(des_state);
