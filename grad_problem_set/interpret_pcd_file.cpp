...
int main(int argc, char** argv) {
    ...
    // Do the same thing but for the pins at the pin height...
   ...
   // double min_x = -0.005231;
    //double max_x = 0.315519;
    zval_black = 0.02; // in ouput image, assign intensity=0 at this z-value (or less)
    zval_white = 0.03; //in output image, assign intensity=255 at this z-value (or above)      
    //int u, v;
    //point-cloud data is in units of meters, but image will be indexed by pixels;
    // scale factor converts meters to pixels; will need to go back in other direction as well
    //double meters_to_pixels = 110; //100 //convert pointcloud data to pixel numbers
    cv::Mat pin_depthmap_image(Nu, Nv, CV_8U, cv::Scalar(0)); //create image, set encoding and size, init pixels to default val
    //uchar gray_level;
    //double x, y;
    //for each pointcloud point, compute which pixel it maps to and find its z value
    for (int ipt = 0; ipt < npts_cloud; ipt++) {
        ...
        if ((z == z)&&(x == x)&&(y == y)) { // test for Nan 
            ...
            if ((u > 0)&&(u < Nu - 1)&&(v > 0)&&(v < Nv - 1)) {
                pin_depthmap_image.at<uchar>(u, v) = gray_level;
            }
        }

    }
    //ROS_INFO("min_x,max_x, min_y,max_y = %f, %f, %f, %f",min_x,max_x,min_y,max_y);
    std::cout << "pin output image size: " << pin_depthmap_image.size().height << " , "
            << pin_depthmap_image.size().width << std::endl; //initially, 0x0
    //save the synthesized  image to  disk
    cv::imwrite("pin_output.bmp", pin_depthmap_image);
    ...
    /*
    int Ntu = 17; //define template 10x10 (arbitrary)
    int Ntv = 17; 
    int tu_c = 8; //(8,8) is central pixel of template 17x17 template
    int tv_c = 8;
    int template_u_halfwidth = 8; //template is +/-5 pixels about center
    int template_v_halfwidth = 8;

    int Rpix = 8; //what is this needed for later??
    cv::Mat loaded_template_img; //(Ntu, Ntv, CV_8U, cv::Scalar(0)); //create image, set encoding and size, init pixels to default val
    loaded_template_img = cv::imread("gasket_template_image.bmp");
    cv::Mat template_img; //(Ntu, Ntv, CV_8U, cv::Scalar(0)); //create image, set encoding and size, init pixels to default val
    loaded_template_img.convertTo(template_img,CV_8U); // might need to fill the fourth field with a fraction...
    //template_img = loaded_template_img;
    //depthcam_snapshot.pcd
    */
    ...
    */
    ...
    //cv::imwrite("loaded_template_img.bmp", loaded_template_img);    
