...
int main(int argc, char **argv) {
    ...
    int random_turn = 0; // will be either 0 or 1 for CCW or CW
    ...
    while(ros::ok()) { // do forever
        ...
        // here if got an alarm; turn a random direction until alarm clears
        
        twist_cmd.linear.x=0.0; //stop moving forward

        random_turn = rand() % 2;
	if (random_turn == 0) {
	   twist_cmd.angular.z=yaw_rate; //and start spinning in place CW
	}
        else {
           twist_cmd.angular.z=-yaw_rate; //and start spinning in place CCW
        }
        
        timer=0.0; //reset the timer
        while(g_lidar_alarm) { // keep turning while we're getting the alarm
          twist_commander.publish(twist_cmd);
          timer+=sample_dt;
          ros::spinOnce();
          loop_timer.sleep();
