...
// this is bad but I'm not getting g_current_pose to work, and this MUST BE CHANGED if orientation is not 0.0 in the beginning
...
void get_yaw_and_dist(geometry_msgs::Pose current_pose, geometry_msgs::Pose goal_pose,double &dist, double &heading) {
 ...
 if (dist < g_dist_tol) { //too small of a motion, so just set the heading from goal heading
   heading = convertPlanarQuat2Phi(goal_pose.orientation);                                  
    ...
    heading = atan2(dy,dx); //I THINK THIS IS WHAT'S WRONG
 }
...
bool callback(example_ros_service::PathSrvRequest& request, example_ros_service::PathSrvResponse& response)
{
    ...
    for (int i=0;i<npts;i++) { //visit each subgoal
        ...
        get_yaw_and_dist(g_current_pose,pose_desired,travel_distance, yaw_desired);  // BROUGHT THIS BACK IN, I THINK I NEED TO FIX IT
        // a quaternion is overkill for navigation in a plane; really only need a heading angle
        // this yaw is measured CCW from x-axis
        // GET RID OF NEXT LINE AFTER FIXING get_yaw_and_dist()                                          done
        ...
        // I COMMENTED THIS OUT. SOMETHING WENT WRONG BELOW    
        ...
        spin_angle = yaw_desired - g_current_yaw; // spin this much
        spin_angle = min_spin(spin_angle);// but what if this angle is > pi?  then go the other way          // MAYBE THIS IS WRONG
        ...
        g_current_yaw = g_current_yaw + spin_angle;  // This is nasty, but I wasn't getting g_current_pose.orientation to give me yaw_current correctly
        
        //FIX THE NEXT LINE, BASED ON get_yaw_and_dist()
        ROS_INFO("Doing move...");
        do_move(travel_distance);  // this is received from get_yaw_and_dist earlier
